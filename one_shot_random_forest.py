...
class OSRandomForest(SegmentationModel):
    def __init__(self, name: str,
        ...
        if os.path.exists(join(ML_SNAPSHOTS_DIR, f'{name}.joblib')):
            self.nnc = joblib.load(join(ML_SNAPSHOTS_DIR, f'{name}.joblib'))
        else:
            self.nnc = RandomForestClassifier(**dtree_args).fit(train_x, train_y)
