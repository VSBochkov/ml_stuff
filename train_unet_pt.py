...
INPUT_SIZE = (224, 224)
BS_TRAIN = 64  # 4  # 24
BS_VAL = BS_TRAIN #int(BS_TRAIN * 1.5)  # BS_TRAIN * 2 - 4
# LEARN_RATES = {
#     0:  1e-3,
#     50: 1e-4,
#     100: 1e-5,
...
for i in range(1, 8):
    LEARN_RATES[i * 250] = LEARN_RATES[0]
    LEARN_RATES[i * 250 + 50] = LEARN_RATES[50]
    LEARN_RATES[i * 250 + 100] = LEARN_RATES[100]
    LEARN_RATES[i * 250 + 150] = LEARN_RATES[150]
    LEARN_RATES[i * 250 + 200] = LEARN_RATES[200]
#LEARN_RATES = {0: 1e-7}

WEIGHT_DECAY = 5e-4
EPOCH_N = 2000 #250
TRAIN_GOLDGOAL_PERIOD = 0 #int(EPOCH_N / 4)
EVAL_EVERY = 1
...
def eval(epoch):
    ...
    if metrics_accumulated['bce'] / epoch_samples < best_bce:
        ...
        checkpoint_path = join(snapshot_dir, best_bce_checkpoint_filename.format(epoch))
    ...
    if metrics_accumulated[MAIN_METRIC + '_loss'] / epoch_samples < best_jaccard_loss:
        ...
        checkpoint_path = join(snapshot_dir, best_jaccard_loss_checkpoint_filename.format(epoch))
        save_checkpoint(model, epoch, checkpoint_path)
    if metrics_accumulated['loss'] / epoch_samples < best_loss:
        best_loss = metrics_accumulated['loss'] / epoch_samples
        best_loss_epoch = epoch
        checkpoint_path = join(snapshot_dir, best_loss_checkpoint_filename.format(epoch))
        save_checkpoint(model, epoch, checkpoint_path)
    if metrics_accumulated[MAIN_METRIC + '_acc'] / epoch_samples > best_acc:
        ...
        checkpoint_path = join(snapshot_dir, best_acc_checkpoint_filename.format(epoch))
        save_checkpoint(model, epoch, checkpoint_path)
    if train_acc + metrics_accumulated[MAIN_METRIC + '_acc'] / epoch_samples > best_trainval_acc:
        ...
        checkpoint_path = join(snapshot_dir, best_trainval_acc_checkpoint_filename.format(epoch))
        save_checkpoint(model, epoch, checkpoint_path)
